...
public class MainController implements Initializable {
    ...
    private static final List<FileChooser.ExtensionFilter> EXTENSION_FILTERS = FileExtension.valuesAsList().stream()
            .map(FileExtension::getExtensionFilter).collect(Collectors.toList());
    ...
    public void openFile(ActionEvent actionEvent) {
        ...
        if (openedFile != null) {
            this.allStudents.setAll(StudentUtils.loadStudents(openedFile));
            this.studentsToSave.setAll(StudentUtils.loadStudents(openedFile));
            allStudents.stream().sorted(Comparator.comparingInt(Student::getId).reversed()).findFirst().ifPresent(student -> ID_GENERATOR.set(student.getId()));
    ...
    public void saveFile(ActionEvent actionEvent) {
        if (studentsToSave.getSize() != allStudents.getSize() || studentsToSave.stream().anyMatch(student -> !allStudents.contains(student))) {
            if (openedFile != null) {
                StudentUtils.saveStudents(this.studentsToSave.get(), openedFile);
            }
        }
    }
    ...
    public void saveFileAs(ActionEvent actionEvent) {
        ...
        if (newFile != null) {
            StudentUtils.saveStudents(this.studentsToSave.get(), newFile);
        }
    }
